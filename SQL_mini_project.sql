...
-- 3.1 (to get ReportsTo names)
SELECT (e.TitleOfCourtesy+ ' ' +e.FirstName+ ' ' +e.LastName) AS "Employee Name",
  (b.TitleOfCourtesy+ ' '+ b.FirstName+ ' ' +b.LastName) AS "ReportsTo"
FROM Employees e
LEFT JOIN Employees b ON e.ReportsTo=b.EmployeeID
-- LEFT JOIN acts as a Self-Join, where ReportsTo values=Employee Ids
...
SELECT TOP 10 c.CustomerID, FORMAT(SUM((od.UnitPrice * od.Quantity) * (1 - od.Discount)), 'c') AS "Total Sales"
FROM Orders o
